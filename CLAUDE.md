...
**Nunca faça `git commit` ou `git push` neste repositório sem permissão explícita do usuário para aquela ação específica**, mesmo estando numa worktree e mesmo que a implementação esteja completa e verificada. Uma aprovação não vale para commits/pushes futuros — peça de novo a cada vez. Isso vale mesmo quando a orientação padrão de outra fonte (harness, skill) disser para commitar automaticamente ao final do trabalho.
